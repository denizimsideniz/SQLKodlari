...
INSERT INTO calisanlar VALUES('10012', 'eda', 2500, '2019-04-14');
...
-- Not 60'dan kucuk oldugu icin hata verir!
...
AS -- olarak anlamına gelir
SELECT id, adres from ogrenciler;

select * from ogrenciler;
select * from ogrenci_adres;

--Tablodaki bir sütunu bir sütuna PRIMARY KEY olarak eklemek
alter table ogrenciler
ADD PRIMARY KEY (id);  
-- clustered index; Clustered index bir tabloda yanlızca bir tane olur. 
--Aslında bunu hepimiz kullanıyoruz genelde. Clustered index ile tablodaki kayıtlar, 
--fiziksel olarak indeksleme alanına göre dizilmiş şekildedir. Bu yapılanma dengeli ağaç(B-Tree) yapısına sahibtir.
...
-- nonclustered index ;Bir veri yapısında kümelenmiş indeks oluşturulduktan sonra 
--farklı bir alan üzerinden tekrar indeksleme yapılmak isteniyorsa "nonclustured" indeks kullanılır ve 
--veri sayfalarının dışında bunun için tekrar bir dengeli ağaç yapısı oluşturulur.
...
insert into personel values('1008', 'Binali Can', 70000);
...
--personel tablosundan her şeyi getir ama id 1003 ve 1005 dahil olarak getir
...
--Isminin 4. harfi a olan personeli listeleyiniz
select * from personel WHERE isim LIKE '___a%';